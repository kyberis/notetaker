...
export type WillMcpAuth = { userId: string; tokenId: string; scopes: string[] };
...
export async function verifyWillMcpBearer(
  ...
  return { userId, tokenId: intro.tokenId ? `acc:${intro.tokenId}` : "acc:unknown", scopes: intro.scopes };
}
